...
export class TimeUnitModule {
    ...
    registerUnitDeltaGenerators(config: Record<string, TimeUnitDateDeltaGenerator>) {
        for (const [key, value]  of Object.entries(config)) {
            this.registerUnitDeltaGenerator(key, value);
        }
    }

    registerUnitDeltaGenerator(unit: string, generator: TimeUnitDateDeltaGenerator) {
        // TODO : warn about the override, if a generator with the same name exists.
        this._units[unit] = generator;
    }

    getUnitDeltaGenerator(unit: string): TimeUnitDateDeltaGenerator {
...
export function unitModule(): TimeUnitModule {
